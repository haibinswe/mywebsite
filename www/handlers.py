...
async def api_register_user(*, email, name, passwd):
    ...
    print('return value of api/users:', r)
    return r
...
@post('/api/blogs')
async def api_crete_blog(request, *, name, summary, content):
    '''
    创建日志
    :param request:客户端请求
    :param name: 日志名称
...
async def authenticate(*, email, passwd):
    ...
    users = await User.findAll('email=?', [email])
    if len(users) == 0:
        raise APIValueError('email', 'Email not exitst.')
    user = users[0]
    # check passwd:
    sha1 = hashlib.sha1()
