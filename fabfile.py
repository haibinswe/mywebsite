...
env.user = 'root'
env.password = 'HHBhhb19971210'

env.sudo_user = 'root'
env.hosts = ['39.107.81.78']
env.password = 'HHBhhb19971210'
...
_TAR_FILE = 'dist-mywebsite.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/mywebsite'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-mywebsite-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick mywebsite > %s' % (db_user, db_password, f))
...
def deploy():
    ...
    with settings(warn_only=True):
        sudo('supervisorctl stop mywebsite')
...
def restore2local():
    ...
    sqls = [
        ...
        'grant select, insert, update, delete on mywebsite.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s mywebsite < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
